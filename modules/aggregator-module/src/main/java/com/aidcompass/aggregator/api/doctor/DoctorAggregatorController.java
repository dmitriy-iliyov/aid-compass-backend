...
public class DoctorAggregatorController {
    ...
    public ResponseEntity<?> getDoctorsBySpecialization(@ModelAttribute @Valid DoctorSpecializationFilter filter) {
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(service.findAllApprovedBySpecialization(filter));
    }


    @GetMapping("/cards/names")
    public ResponseEntity<?> getAllDoctorsByNamesCombination(@ModelAttribute @Valid NameFilter filter) {
        return ResponseEntity
                .status(HttpStatus.OK)
    ...
    public ResponseEntity<?> delete(@AuthenticationPrincipal PrincipalDetails principal,
                                    @PathVariable("password") @NotBlank(message = "Password can't be empty or blank!")
