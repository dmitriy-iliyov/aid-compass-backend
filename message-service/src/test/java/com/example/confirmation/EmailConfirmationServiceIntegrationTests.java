...
public class EmailConfirmationServiceIntegrationTests {
    ...
    void sendConfirmationMessage_shouldSaveTokenToRedis() throws MessagingException {
        ...
        UUID token = UUID.fromString(Strings.ascii(Base64.getUrlDecoder().decode(messageDto.text().split("token=")[1].getBytes())));
    ...
    void validateConfirmationToken_shouldConfirmUser_whenTokenIsValid() throws MessagingException {
        ...
        String tokenEncoded = sentMessage.text().split("token=")[1];
    ...
    @Test
    @DisplayName("IT: validateConfirmationToken() should throw when token is invalid or missing")
    void validateConfirmationToken_shouldThrowException_whenTokenIsInvalid() {
        ...
        assertThrows(BaseInvalidConfirmationTokenException.class, () -> emailConfirmationService.validateConfirmationToken(encoded));
    }

    @Test
    @DisplayName("IT: validateConfirmationToken() should throw when token is malformed")
    ...
    void tokenShouldExpireAfterTTL() throws Exception {
        ...
        ArgumentCaptor<MessageDto> captor = ArgumentCaptor.forClass(MessageDto.class);
        verify(messageService).sendMessage(captor.capture());
        String tokenEncoded = captor.getValue().text().split("token=")[1];

        Thread.sleep(11000);

        assertThrows(BaseInvalidConfirmationTokenException.class, () -> emailConfirmationService.validateConfirmationToken(tokenEncoded));
    }
}