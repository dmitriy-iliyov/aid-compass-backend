...
public class PasswordRecoveryControllerIntegrationTests {
    ...
    @Test
    @DisplayName("IT: recoverPassword() should return NO_CONTENT on valid token")
    void recoverPassword_whenTokenValid_shouldReturnNoContent() throws Exception {
        String token = "valid-token";
        String password = "new-password";

        doNothing().when(passwordRecoveryService).recoverPassword(token, password);
        ...
        verify(passwordRecoveryService, times(1)).recoverPassword(token, password);
        verifyNoMoreInteractions(passwordRecoveryService);
    }

    @Test
    @DisplayName("IT: recoverPassword() should return BAD_REQUEST when token is invalid")
    void recoverPassword_whenTokenInvalid_shouldReturnBadRequest() throws Exception {
        String token = "invalid-token";
        String password = "new-password";

        doThrow(new BaseInvalidPasswordRecoveryTokenException()).when(passwordRecoveryService).recoverPassword(token, password);
        ...
        mockMvc.perform(patch("/api/password-recovery/recover")
                ...
                .andExpect(jsonPath("$.properties.errors[0].message")
                        .value("Recovery token is invalid!"));
