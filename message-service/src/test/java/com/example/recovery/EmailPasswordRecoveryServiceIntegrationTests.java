...
import com.example.exceptions.models.BaseInvalidPasswordRecoveryTokenException;
...
public class EmailPasswordRecoveryServiceIntegrationTests {
    ...
    void sendRecoveryMessage_shouldStoreTokenInRedis() throws MessagingException {
        ...
        UUID token = UUID.fromString(Strings.ascii(Base64.getUrlDecoder().decode(message.text().split("token=")[1].getBytes())));
        Optional<String> emailInRedis = redisPasswordRecoveryRepository.findAndDeleteByToken(token);
    ...
    void sendRecoveryMessage_shouldNotStoreTokenIfException() throws MessagingException {
        ...
        UUID token = UUID.fromString(Strings.ascii(Base64.getUrlDecoder().decode(msg.text().split("token=")[1].getBytes())));
    ...
    void recoverPassword_shouldRecover_whenValidTokenProvided() throws MessagingException {
        ...
        String encodedToken = captor.getValue().text().split("token=")[1];
    ...
    @Test
    @DisplayName("IT: recoverPassword() should throw if token not found")
    void recoverPassword_shouldThrow_whenTokenInvalid() {
        ...
        assertThrows(BaseInvalidPasswordRecoveryTokenException.class, () -> recoveryService.recoverPassword(encoded, "123"));
    }

    @Test
    @DisplayName("IT: recoverPassword() should throw if token malformed")
    void recoverPassword_shouldThrow_whenTokenMalformed() {
        String brokenToken = "non-base64-token";
    ...
    void tokenShouldExpireAfterTTL() throws Exception {
        ...
        String token = captor.getValue().text().split("token=")[1];

        Thread.sleep(10010);

        assertThrows(BaseInvalidPasswordRecoveryTokenException.class,
                () -> recoveryService.recoverPassword(token, "irrelevant"));
    }
}