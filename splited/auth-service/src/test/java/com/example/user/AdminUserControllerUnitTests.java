...
class AdminUserControllerUnitTests {
    ...
    void updatedUserById_shouldUpdateUser() {
        ...
        ResponseEntity<?> response = adminUserController.updatedUserById(userId.toString(), userUpdateDto);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
        verify(userFacade).update(userId, userUpdateDto);
    }
    ...
    void deleteUserById_shouldDeleteUser() {
        ...
        ResponseEntity<?> response = adminUserController.deleteUserById(userId.toString(), password);
