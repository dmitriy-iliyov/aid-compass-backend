...
class UserControllerUnitTests {
    ...
    void updateUser_whenValidRequest_shouldReturn200() {
        UUID id = UUID.randomUUID();
        doNothing().when(userFacade).update(any(UUID.class), any(UserUpdateDto.class));

        ResponseEntity<?> response = userController.updatedUser(id, new UserUpdateDto("testu@example.com", "test_oihugyuiokiuhhijpassword"));
