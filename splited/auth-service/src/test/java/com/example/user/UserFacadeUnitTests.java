...
import com.example.exceptions.not_found.*;
import com.example.rest_clients.RecoveryRequestDto;
...
public class UserFacadeUnitTests {
    ...
    void confirmByEmail_shouldThrow_whenUnconfirmedUserNotFound() {
        ...
        doThrow(NotFoundException.class).when(userService).confirmByEmail(email);
    ...
    void systemFindByEmail_shouldReturnFromUserService() throws NotFoundException {
        String email = "test@example.com";
        SystemUserDto expected = new SystemUserDto(UUID.randomUUID(), email, "pass", List.of(), Instant.now(), Instant.now());
    ...
    void systemFindByEmail_shouldFallbackToUnconfirmed_ifNotFoundInUserService() throws NotFoundException {
        String email = "notfound@example.com";
        SystemUserDto fallback = new SystemUserDto(UUID.randomUUID(), email, "pass", List.of(), Instant.now(), Instant.now());

        doThrow(NotFoundException.class).when(userService).systemFindByEmail(email);
    ...
    void update() {
        ...
        doNothing().when(userService).update(any(SystemUserUpdateDto.class));
