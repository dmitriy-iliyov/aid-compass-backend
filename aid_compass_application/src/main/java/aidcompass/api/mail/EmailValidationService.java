...
import com.example.clickerapi.services.user.UserService;
...
public class EmailValidationService implements ConfirmationService {
    ...
    private EmailValidationEntity generateToken(String recipientMail){
        UUID token = UUID.randomUUID();
        Instant expiresAt = Instant.now().plus(tokenTtl);
        return new EmailValidationEntity(recipientMail, token, expiresAt);
    }

    @Transactional
    public void sendConfirmationMessage(String recipientMail) {

        EmailValidationEntity entity = generateToken(recipientMail);
    ...
    public boolean validateConfirmationAnswer(String input) {
        ...
        EmailValidationEntity entity = validationRepository.findByToken(inputToken)
